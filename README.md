...
④ 执行占卜（_run_reading）【tarot_core.py 抽牌渲染 · interpret.py + hardening.py 解读】
   ...
   ├─ 渲染牌面图：官方素材 + 正逆位（线程池，临时图 30 秒自动清理）；拼图背景改本签随机一张牌面做底（cover 放大 + 深藏青遮罩）；/单抽 与每日牌运渲染专属海报卡（牌面 + 日期 + 池内当日签文 + 署名，`output.daily_card` 可关，关闭或失败回退普通牌面图）
...
- `card_render.py` 负责拼图渲染：背景从本签抽出的牌里随机挑一张的牌面做底——cover 放大 + 深藏青遮罩（与今日牌运卡同一套风味），所有牌统一白边卡牌样式（素材底色差异不影响观感），逆位整张旋转 180°
