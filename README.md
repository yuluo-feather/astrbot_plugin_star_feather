...
- 🔮 **78 张牌全内置**：本羽把 22 张大阿卡纳 + 56 张小阿卡纳（权杖 / 圣杯 / 宝剑 / 星币）一张不落地收了进来，每张牌都有中英文名与分层牌义——正位「关键词 + 事件走向 + 心法建议」，逆位「关键词 + 能量状态 + 能量卡点 + 转身建议」
- 🃏 **四种牌阵**：羽签、羽时三刻、羽镜、恋羽十字——经典名（单张问询 / 时间之流 / 圣三角 / 恋人十字）输入同样识别
- 🧠 **智能选阵**：本羽瞄一眼问题的关键词，就替你挑好最合适的牌阵，不用你操心
- 🤖 **AI 深度解读**：调用 LLM 深度解读（可按 `ai.ai_provider` 指定专用模型），按「每张牌一段 + 总结段」直白说结果；调用失败自动回退内置牌义，不会卡住占卜——这一环本羽盯得最紧，别的可以慢慢来，占卜不能停在半路
- 🗣️ **牌灵人设化**：AI 解读由牌灵开口讲——傲娇 / 温柔 / 神秘三格口吻（`ai.persona` 可切换，`off` 恢复中立），各带口吻硬约束（整段至少两处人设语气）与签文风格样板（牌灵的话按人设生成）；`random` 一签固定随机一种、本签全程不变；只改口吻，解读内容与结构不变
...
- 🎴 **幻星集官方素材**：78 张牌面 + 官方牌背全在 `assets/` 里，白边卡牌统一风格，逆位翻转呈现——牌面好看，占卜才体面
...
| 功能 | 命令 | 描述 |
|------|------|------|
| 🎴 智能占卜 | `/占卜 [问题]` | 根据问题关键词自动匹配牌阵，抽牌并输出牌面图片；带「运势 / 运气」等词走今日固定牌运（因果归因问法——「是因为运势太差吗」——除外，走自由随机） |
| 🃏 快速单抽 | `/单抽` | 今日牌运：一天一张固定牌，零点刷新；解读按主题分区、当天内固定（同一主题反复问不换说法）；输出专属海报卡（牌面+日期+牌灵的话+署名） |
...
如果你同时跑了好几个 bot（多个账号 / 多个实例），请记住：群聊消息是广播给群里所有机器人的，每个 bot 都独立判断要不要响应——所以本羽先把丑话说在前面：
...
> 💫 经典名（单张问询 / 时间之流 / 三张时间线 / 圣三角 / 恋人十字）本羽都留着呢，不用担心叫不惯。
...
③ 抽牌分流（_pick_reading）【spreads.py 选阵 · daily.py 今日固定】
   ...
   │       事件归因式——「手指被夹了，是因为运势太差吗」——不算运势查询，走自由随机；
   │       泛时间词仅白名单两类——「最近怎么样/这个月如何」（泛问句式）
   │       与「每日塔罗/今天来一签」（签文类）——「最近总是失眠/今天下午开会吗」走具体占卜）
   └─ 具体问题（感情/事业/事件归因/日程事件等）→ 自由随机
         ├─ 显式指定阵名（/占卜 圣三角 考研如何）> 关键词匹配 > 内容推断
         │     （时间线词 + 关系语义：我们的未来/她未来会爱我吗 → 关系阵；纯时间线：我的未来 → 羽时三刻）
         └─ 都猜不中 → 兜底「羽时三刻」
   │
   ▼
④ 执行占卜（_run_reading）【tarot_core.py 抽牌渲染 · interpret.py + hardening.py 解读】
   ├─ 洗牌提示：多张牌阵随机发一句「✨ 洗牌中……」(shuffle_lines 可关)
   ├─ 渲染牌面图：官方素材 + 正逆位（线程池，临时图 30 秒自动清理；今日牌运海报卡 300 秒）；拼图背景改本签随机一张牌面做底（cover 放大 + 深藏青遮罩）；/单抽 与每日牌运渲染专属海报卡（牌面 + 日期 + 池内当日签文 + 署名，`output.daily_card` 可关，关闭或失败回退普通牌面图）
   ├─ 牌灵的话：所有牌阵都有一句牌灵开口——AI 按人设现生成（贴合本签牌面与主题，同人同日同牌组当天同句、次日换新），AI 不可用回退池内当日签文；裸句直出（无「牌灵的话：」标签与引号），排在牌面图之后、解读之前
   └─ 生成解读：
...
⑤ 发送结果（_deliver）【deliver.py 发送编排 · settings.py 配置生效】
   ├─ 呈现顺序：牌面图 → 牌灵的话 → 解读段（每张一段 + 总结）→ 免责声明（合并转发节点序同为 牌面 → 牌灵的话 → 解读段）
...
**三个入口共用同一套②~⑤流程**，所以无论在哪个入口触发，抽牌规则、限流、发送方式、兜底完全一致；差别只在触发方式，以及收尾句的出口——命令入口由本羽直接直发固定文案，自然语言入口由模型原样说出（同一文案池，风格一致）。

## 🎛️ 配置项

本羽把配置项按用途分了组，在 AstrBot 插件管理界面里配：
...
| `ai.ai_fail_cooldown` | int | `60` | AI 解读失败后的冷却时间（秒），按 Provider 粒度：某个模型挂掉只冷却它自己，其余模型照常服务；设为 0 关闭冷却。 |
| `ai.persona` | select | `random` | 牌灵人设（AI 解读口吻）：`off`-中立（与旧版一致）/ `tsundere`-傲娇 / `gentle`-温柔 / `mystic`-神秘 / `random`-每签固定随机一种（本签全程不变，下一签才换）。只改口吻，解读内容与输出结构不变。 |
...
| `output.daily_card` | bool | `true` | 今日牌运卡海报。开启后 `/单抽` 与每日牌运输出竖版海报卡（牌面+日期+牌灵的话+署名）；关闭则回到普通牌面图，牌灵的话仍随解读出现。 |
...
**依赖**：牌面渲染需要 [Pillow](https://pypi.org/project/pillow/)（AstrBot 环境通常自带；若缺失可执行 `pip install -r requirements.txt`）。字体缺字回退校验需要 [fonttools](https://pypi.org/project/fonttools/)，已一并列入 `requirements.txt`——就这两样，本羽没往里塞多余的。
...
- 渲染前对文本做字形覆盖校验：内置子集缺字时自动回退到能覆盖该文本的系统字体——校验自带静态字形清单兜底，`fonttools` 缺失也不影响回退（仍列入 requirements，首选动态校验）。当前牌库文案全覆盖子集，此机制是为未来新增生僻字牌义兜底
...
- AI 解读采用「候选链 + 超时 + 失败冷却」：按指定模型（`ai.ai_provider`）→ 当前会话 Provider → 全局默认 → 全部已加载 Provider 依次尝试（按 id 去重），单个请求超时 `ai_timeout`（默认 30s），全部失败回退本地牌义；失败按 **Provider 粒度**冷却 `ai_fail_cooldown`（默认 60s）——某个模型挂掉只冷却它自己，其余模型照常服务，避免反复空等
...
- **「固定牌 + 随题解读」的设计取舍**：AI 解读让牌义第一次摆脱「一张牌 = 一段固定文案」的绑定——传统塔罗是字典式对应（牌 → 固定牌义），AI 解读则把【牌面】与【你的问题】一起送进模型，同一张牌面对不同主题（感情 / 事业 / 学业）会生成贴题的不同解读。星羽没有放任它每次都不一样，而是把变化锁进确定性框架：抽牌按（用户, 日期）确定性命中（问一百遍是同一张），解读按主题分区、当天内固定（同主题反复问不换说法）——占卜这件事，答案越不稳定越像假的
...
- 修复：纯文字模式的逐张牌义不再只剩最后一张——AI 不可用时的本地兜底此前只输出牌阵的最后一张，现在每张牌各出一段

#### v0.6.3

- 变更：适配 AstrBot 4.28——AI 解读的模型选择改用框架推荐的异步路径，升级 AstrBot 4.28 后占卜解读照常工作，不再依赖框架已弃用的同步接口

#### v0.6.2

- 修复：AI 不可用时牌义不再丢失——文字模式下关闭 AI 或解读失败时，此前只显示牌灵的一句话、逐张牌义缺失，现在照常给出每张牌的牌义兜底
...
- 新增：牌灵人设化——AI 解读口吻三选一（傲娇/温柔/神秘）或每签随机（`ai.persona`，off 恢复中立语气），人设卡含口吻硬约束与签文风格样板，内容与结构不变；牌灵的话一并人设化——所有牌阵（羽签/羽时三刻/羽镜/恋羽十字/每日牌运）都有一句牌灵开口（AI 按人设生成，同人同日同牌组当天同句，AI 不可用回退池内），裸句直出（无标签与引号）、一句话说人话（比喻一读就懂），置于牌面图后、解读前
- 新增：今日牌运卡——`/单抽` 与每日牌运输出专属海报卡（牌面+日期+签文+署名），卡面印池内当日固定签文；渲染失败自动回退普通牌面图；自带开关 `output.daily_card`（默认开）；视觉语言顺带普及——羽时三刻/恋羽十字等拼图背景也改为本签随机一张牌面做底（3 选 1 / 4 选 1，同牌运卡风味）
- 修复：卡面字体回退加固——字体校验组件未就绪（如依赖安装中）时牌面/牌运卡不再显示方块字，内置字体缺字一律回退系统字体
...
- 占卜结果仅供娱乐参考，请理性看待 🍀 牌给的是方向，路还得你自己走——本羽只负责把牌摆明白
...
- 遇到问题、想要新功能？欢迎提交 [Issue](https://github.com/yuluo-feather/astrbot_plugin_star_feather/issues) 或 Pull Request——本羽每一份都会认真看
